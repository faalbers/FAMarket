...
- Filter-system robustness & scoring mode (added 2026-06-27, parked — surfaced by the `emerging_dominators.filt` "0 results" investigation). **Why these came up:** a `/make_filters`-built filter returned zero symbols while a free-form agent given the same brief + same `analysis.db` produced a ranked top-20. Root cause was a `market_cap` unit-scale bug (band written as `1000–80000` against a raw-dollar column) compounded by hard AND-gating + N/A-silently-fails. The unit bug and N/A handling were fixed in the `make_filters` skill (unit-convention table + mandatory dry-run + broadened N/A fallbacks, 2026-06-27); these three are the **larger, app-side** follow-ups we deliberately deferred:
  - **Live per-block diagnostics in the Filter page** — show how many in-scope symbols each block matches *as you build*, plus an auto "why zero results" breakdown when a run returns nothing (which block collapsed the set). The post-hoc **Filter Fail** tool already proves the value; this brings the same insight live, so a units/over-tight mistake is visible immediately instead of after a 0-result run. Cheap-ish: reuse `filter_engine._block_mask` per block over the in-scope frame.
  - **Human-unit input / display** — accept `10B`/`30B` shorthand and render `$10.5B` next to `$`-unit value boxes (hint already carries `unit`). **Why:** makes the scale mistake that caused this bug *impossible for a human too* — you can't fat-finger nine zeros if you type `10B`. Pairs with the skill-side unit table so both the human and the generator are unit-safe.
  - **Ranking / soft-score filter mode** — let blocks contribute to a composite score and return a top-N **ranked** shortlist instead of all-or-nothing AND gates. **Why:** the user's real goal ("a list sorted descending by score") is what the free-form agent delivered by *scoring & ranking*, tolerating one weak axis; pure conjunctive gating excludes a name on any single borderline metric. Reuses existing infra (`*_goodness`, category scores, Overall) — no new math — so a filter becomes a coarse pre-screen feeding a ranked view. Largest of the three; needs its own design pass.
- Guidance-vs-actual tracking → management-credibility signal (added 2026-08-23, parked — the feasibility spike is BUILT but has never been run). **The idea** (`dev_docs/FAMarket_Epansion.md` Topic 1): track the gap between the forward guidance management issues and what actually gets reported later, per company. Consistent over-delivery vs. over-promising becomes a codable credibility metric — something no ratio expresses. Topic 1 records exactly one blocker: *"needs an earnings-call-transcript/press-release data source, not yet identified."*
  - **`scripts/test_guidance_extraction.py` exists to break that blocker** and answers two questions in one run: (1) can the guidance text be had free — it pulls the most recent 8-K press-release exhibit from SEC EDGAR (free, email header only) and regex-locates the guidance paragraph; (2) can it become structured numbers and at what price — it sends the excerpt to Claude Haiku against a fixed JSON schema (`has_guidance`, `period`, revenue low/high, EPS low/high, `confidence`) and computes the REAL per-call cost from `response.usage`, deliberately so it reconciles against the Anthropic console instead of being an estimate. Defaults to FDX + NKE, both reliable quantitative guiders.
  - **Status: unrun.** Its output file `dev_docs/guidance_extraction_test_results.json` does not exist, and Topic 1 still reads "not yet identified" — so the feasibility question was never closed. The script is not wired into `analysis_layer`, `filter_registry.py` or `param_hints.py`; it is a standalone spike.
  - **Why it is worth returning to rather than deleting** (decided 2026-08-23 during a dead-code sweep): it is not leftover scaffolding from finished work, it is an unrun experiment for a still-open question — and that question feeds the doc's own **priority build**, the Claude-generated parameter/filter injection pipeline, which names "guidance credibility" as one of its target qualitative-judgment areas. The fiddly half (EDGAR 8-K fetch-and-locate plumbing) is already solved.
  - **PICK-UP TRIGGER — cheap, do it early in any development session that touches forward-looking data, the expansion topics, or the LLM-parameter pipeline.** Run `python -m scripts.test_guidance_extraction` (needs `ANTHROPIC_API_KEY` in `.env`; costs a few cents). The outcome either closes Topic 1's data-source question and unlocks the credibility metric, or shows the approach does not work and the idea can be retired honestly. Until it is run, everything downstream of it is guesswork.
