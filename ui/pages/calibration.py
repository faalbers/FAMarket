...
Built into the UI (not a separate script). The `trend` column in analysis.db is a
peak-detection classification (HH/HL vs LL/LH on swing highs/lows found by
`scipy.signal.find_peaks`). Two knobs control how sensitive that detection is —
`PEAK_PROMINENCE` (min swing size, as a fraction of the window's mean price) and
`PEAK_DISTANCE` (min trading days between peaks) — and they ship as untuned
placeholders in `config/settings.py`.

This page lets Frank slide those two knobs and watch the detected peaks move on a
real price chart, flip through a spread of representative stocks (clear-trend /
choppy / volatile / calm — chosen by price behavior, not the fundamental-growth R²),
and Save the values to `settings.local.json` (the same override file the Settings
page writes). The detection here calls the *same* `technical.trend_signals` the
analysis pipeline uses, so the peaks shown are exactly the ones a run would find.

Changing the knobs only affects the `trend` column on the NEXT analysis run — it
does not retroactively re-label existing rows.
"""
...
# page
# --------------------------------------------------------------------------- #
st.title("Peak Detection Calibration")
st.caption(
...
_pts = lambda idx: [[_win["date"].iloc[i].strftime("%Y-%m-%d"),
...
if _save[0].button("💾 Save settings", type="primary", disabled=not _changed,
