...
Rotation strategy is one log per fetch run: roll_log() snapshots the current log
to famarket.prev.log (a single copy, overwritten every run) and starts a fresh
famarket.log. The fetch orchestrator calls it at the top of every run, so both
the CLI runner and the Streamlit Fetch Control panel behave identically.
...
def roll_log() -> None:
    """Snapshot the current log to a single prior copy, then start fresh.

    Each fetch run begins with an empty log while the immediately previous run's
    log is preserved as ``famarket.prev.log`` — exactly one copy, overwritten
    every run (no version history). Safe whether or not logging is already
    configured: any handler holding LOG_FILE open is closed first (so the file
    can be swapped on Windows) and re-created against the fresh file.
    """
    global _configured
    ...
    if log_file.exists() and log_file.stat().st_size > 0:
        prev = log_file.with_name(f"{log_file.stem}.prev{log_file.suffix}")
        shutil.copy2(log_file, prev)  # overwrite any existing prior copy
